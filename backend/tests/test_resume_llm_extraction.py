...
def test_deepseek_resume_extraction_accepts_only_source_locked_facts(monkeypatch) -> None:
        ...
        lambda **_: _result(
            {
                ...
                "tools_platforms": [
                    {
                        "value": "PyTorch",
                        "evidence_quote": "项目经历：移动机器人导航项目，使用 PyTorch 完成模型训练与验证。",
                    }
                ],
    ...
    assert facts["parser_version"] == "candidate_profile_deepseek_v2_seven_dimensions"
    ...
    dimensions = facts["structured_facts"]["profile_dimensions"]
    assert dimensions["job_responsibilities"][0]["value"] == "负责感知模块开发"
    assert dimensions["tools_platforms"][0]["value"] == "PyTorch"
    assert dimensions["application_scenarios"][0]["value"] == "移动机器人"
    assert dimensions["generic_capabilities"] == []
