...
def get_taxonomy_tree(
    ...
    rows = db.execute(
        ...
        .order_by(TechnologyNode.level_code, TechnologyNode.technology_code)
    ).all()
    ...
    if tids:
        ...
        # referenced_role_cluster_count 要数的是「多少个岗位簇要求该技术」，
        # 但岗位版本要求表上没有簇标识——簇与岗位是通过 rel_job_cluster_role 关联的，
        # 因此需经岗位版本回到岗位，再落到簇。
        c_rows = db.execute(
            select(
                JobRoleVersionRequirement.technology_node_id,
                func.count(distinct(JobClusterRole.job_cluster_version_id)),
            )
            .join(
                JobRoleVersion,
