...
LLM_PARSER_VERSION = "candidate_profile_deepseek_v2_seven_dimensions"
PROMPT_VERSION = "resume_fact_extraction_v3_seven_dimensions"
...
def extract_resume_facts(text: str) -> dict:
        ...
        return {
            **rule_facts,
            "structured_facts": {
                ...
                "work_experiences": [],
                "projects": [],
                "profile_dimensions": _empty_profile_dimensions(),
            },
            "skill_mentions": None,
            "extraction": {
    ...
    return {
        ...
        "structured_facts": {
            ...
            "projects": projects,
            "profile_dimensions": {
                "job_responsibilities": validated["job_responsibilities"],
                "required_skills": validated["skills"],
                "tools_platforms": validated["tools_platforms"],
                "education_major": education,
                "work_experience": experiences,
                "application_scenarios": validated["application_scenarios"],
                "generic_capabilities": validated["generic_capabilities"],
            },
        },
...
def _empty_profile_dimensions() -> dict[str, list]:
    """The seven graph dimensions used by the deterministic question planner."""
    return {
...
def _system_prompt() -> str:
    return (
        ...
        "\"evidence_quote\":\"\"}],"
        "\"job_responsibilities\":[{\"value\":\"\",\"evidence_quote\":\"\"}],"
        "\"skills\":[{\"raw_name\":\"\",\"normalized_keyword\":\"\"," 
        "\"task\":\"\",\"action\":\"\",\"time\":\"\",\"result\":\"\"," 
        "\"proficiency\":\"了解|掌握|熟练|精通|未说明\"," 
        "\"evidence_quote\":\"\"}],"
        "\"tools_platforms\":[{\"value\":\"\",\"evidence_quote\":\"\"}],"
        "\"application_scenarios\":[{\"value\":\"\",\"evidence_quote\":\"\"}],"
        "\"generic_capabilities\":[{\"value\":\"\",\"evidence_quote\":\"\"}]}。"
        "岗位职责提取候选人实际承担的职责；工具平台提取实际使用的软件、框架、设备或平台；"
        "应用场景提取明确出现的行业、产品或作业场景；通用能力只提取有原文行为证据的沟通协作、"
        "分析解决问题、项目推进、质量或交付能力。没有内容时返回空字符串或空数组。"
    )
...
def _validate_payload(text: str, payload: dict) -> dict:
    ...
    job_responsibilities = _validated_records(
        text,
    ...
    tools_platforms = _validated_records(
        text,
        payload.get("tools_platforms"),
        ("value",),
        warnings,
        "tools_platforms",
    )
    application_scenarios = _validated_records(
        text,
        payload.get("application_scenarios"),
        ("value",),
        warnings,
        "application_scenarios",
    )
    generic_capabilities = _validated_records(
        text,
        payload.get("generic_capabilities"),
        ("value",),
        warnings,
        "generic_capabilities",
    )
    accepted = sum(
        [
            ...
            len(tools_platforms),
            len(application_scenarios),
            len(generic_capabilities),
        ]
    )
    return {
        "name": name,
        ...
        "generic_capabilities": generic_capabilities,
