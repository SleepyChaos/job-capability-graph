...
def _personalized_question_text(
    ...
    if question_code == "job_responsibilities" and target:
        return (
            f"简历中还没有清楚说明与“{target}”相关的职责。"
            f"请选一段经历，说明你负责什么、采取了哪些行动，以及结果。"
        )
    if question_code == "required_skills" and target:
        return (
            f"简历中还没有识别出支撑“{target}”的明确技能。"
            f"请列出实际使用过的核心技能，并说明各自用于什么任务。"
        )
    if question_code == "tools_platforms" and target:
        return (
            f"简历中还没有识别出与“{target}”相关的工具或平台。"
            f"请列出实际使用过的软件、框架、设备或平台及其用途。"
        )
    if question_code == "application_scenarios" and (target or skills):
        return (
            f"简历中还没有明确体现应用场景。"
            f"你的{target or skills}经历主要面向哪些行业、产品或机器人场景？"
        )
    return fallback
