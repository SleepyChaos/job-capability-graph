...
# 「技术成熟 → 岗位需求」的传导时滞先验，单位月。
#
# **这是项目设定的外部先验，不是本系统的测量结果。** 必须与其他数字区分对待：
# 我们尝试过从自有数据标定它并且失败了——截尾夹逼退化（12 个已跨门槛的技术方向
# 里 11 个当前已有岗位需求，未到达组只剩 1 个，构不成夹逼），而「跨越至今已历
# 月数」与「当前 JD 需求」的秩相关只有 0.510，n=12 时 p=0.05 的临界值约 0.587，
# 不显著。更根本的是里程碑为回溯整理：我们因为知道某技术已经火了才去整理它的
# 里程碑，跨越时点因此掺着整理时机，用它推「多久之后会有岗位」是拿结果推原因。
#
# 因此凡是由这个先验推出的时间区间，一律带 `external_prior_not_measured` 标记
# 下发，前端必须显著标注来源，不得与真实计算出的跨越时点混排。
TRANSMISSION_LAG_PRIOR_MONTHS = (12, 36)
