...
TRIPLE_PREDICATES = {"org_has_tech", "cluster_needs_tech", "org_has_cluster", "hierarchy", "dg_membership"}
...
class TripleContradictionAssessment(Base):
    ...
    audit_model: Mapped[str] = mapped_column(String(32), default="composite_v1")  # composite_v1 | pykeen_transe | pykeen_complex
    sample_scope: Mapped[str] = mapped_column(String(16), default="top_200")  # top_200 | top_2000 | full

    subject_kind: Mapped[str] = mapped_column(String(24))  # organization|cluster|technology
    subject_id: Mapped[str] = mapped_column(String(64), index=True)  # org_code | cluster_code | tech_code
    ...
    plausibility_level: Mapped[str] = mapped_column(String(16), index=True)  # low/medium/high/auto_suppressed
    rule_flags: Mapped[dict] = mapped_column(JSON, default=dict)  # {cooccur: 0.3, external_agree: 0.7, taxon_violation: True}
    component_scores: Mapped[dict] = mapped_column(JSON, default=dict)  # {pykeen: 0.2, path_count: 0.4, support: 0.5}
    review_status_code: Mapped[str] = mapped_column(String(24), default="pending_review", index=True)  # pending_review | accepted_as_true | false_positive_edge | redirected_edge
    reviewer_code: Mapped[str | None] = mapped_column(String(64))
    review_note_text: Mapped[str | None] = mapped_column(Text)
    evidence_summary_json: Mapped[dict] = mapped_column(JSON, default=dict)  # supporting_job_count, org_count, etc.
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
