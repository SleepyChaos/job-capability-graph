    ...
    <div className="page-stack">
      ...
      <div className="profile-library-layout">
        ...
        <div className="profile-detail-area">
          ...
          <div className="resume-summary"><div className="resume-file"><div className="file-icon"><FileText size={24} /></div><div><strong>{detail.source_name}</strong><span>{detail.skill_count} 项标准能力 · 完整度 {detail.completeness_score}%</span></div><StatusTag tone="success"><Database size={12} />已入库</StatusTag></div><div className="parse-flow">{['文本准入', '事实抽取', '技术映射', '对话补全', '版本入库'].map((step) => <div key={step} className="done"><i><Check size={13} /></i><span>{step}</span></div>)}</div></div>
