  ...
  return <div className="page-stack capability-home">
    ...
    <div className="graph-home-grid">
      <Panel title="P0 图谱数据状态" subtitle={data ? `目标日期 ${data.relations.target_date}` : '正在读取最新快照'}>
        {error ? <div className="empty-state"><strong>图谱数据加载失败</strong><span>{error}</span></div> : data ? <div className="graph-update-status"><StatusTag tone="success">三类投影可用</StatusTag><strong>所有视图共享数据版本 {data.relations.data_version}</strong><p>关系图限制节点规模，聚类图独立计算重要性与近期活跃度，热力图固定 45 天并保留缺失警告。</p><dl><div><dt>聚类运行</dt><dd>{data.relations.clustering_run_code.slice(0, 16)}</dd></div><div><dt>投影版本</dt><dd>{data.relations.projection_version}</dd></div><div><dt>证据口径</dt><dd>已通过语境校验</dd></div></dl></div> : <div className="empty-state"><strong>正在生成图谱投影</strong><span>三个独立请求并行读取，避免页面加载瀑布。</span></div>}
      </Panel>
