      ...
      {showApproval && detail ? (
        <Modal title="新岗位定义与专项审批" onClose={() => setShowApproval(false)}>
          <div className="role-card-preview">
            <div className="special-review-note"><ShieldCheck size={17} /><span><strong>独立审批流程</strong>本次审批对象是基于数据库证据创新形成的岗位定义，不进入数据审核中心。审批通过将在一个事务中创建正式岗位、首版本与标准 JD。</span></div>
