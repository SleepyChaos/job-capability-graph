import { classificationColor, classificationGuidance, classificationLabels, evidenceBadges, LIBRARY_EVIDENCE_KEY } from '../api/discovery'
...
export function GraphRelationsPage({
  ...
  // 仅候选节点带事实卡摘要；有它侧栏就换成候选自己的证据口径。
  const candidateFacts = selectedNode?.type === 'emerging_candidate' ? selectedNode.candidate_facts : undefined
          ...
          */}
          ...
          {data && hasProjection && hasBusinessRelations ? <div className={`graph-workspace graph-workspace--global${graphLoading ? ' is-refreshing' : ''}`}>
            ...
            <aside className="evidence-inspector relation-job-inspector">{selectedNode ? <>
              <div className="inspector-title"><div><span>{selectedNode.type === 'job_cluster' ? '岗位聚类详情' : selectedNode.type === 'technology_domain' ? '技术领域导航' : selectedNode.type === 'emerging_candidate' ? classificationLabels[selectedNode.classification_code ?? ''] ?? '新岗位候选' : '标准技术能力'}</span><h3>{selectedNode.label}</h3></div><StatusTag tone={selectedNode.type === 'job_cluster' ? 'info' : 'success'}>{selectedNode.domain_code}</StatusTag></div>
              {/* 候选与聚类的指标不是同一套量。「证据 JD / 可查看 JD」问的是「有多少份真实
                  招聘文本支撑」，而外部证据类候选的答案按定义就是 0——照搬会得到两格空数，
                  读者只能读成取数失败。候选改用它自己的证据口径：缺口判定、该路径的证据强度
                  （上游共现次数 / 依据事件条数）、综合证据分。 */}
              {candidateFacts ? <div className="inspector-metric-grid"><div><span>缺口判定</span><strong>{(evidenceBadges[candidateFacts.gap_grade ?? LIBRARY_EVIDENCE_KEY] ?? evidenceBadges[LIBRARY_EVIDENCE_KEY]).label}</strong></div><div><span>{candidateFacts.evidence_label}</span><strong>{candidateFacts.evidence_value}</strong></div><div><span>综合证据分</span><strong>{selectedNode.metrics.candidate_score ?? '—'}</strong></div></div>
              ...
              {candidateFacts ? <section className="job-evidence-section"><div className="section-heading"><h4>候选事实</h4>{candidateFacts.established_month ? <span>锚点 {candidateFacts.established_month}</span> : null}</div>
                {candidateFacts.technology_names.length ? <><h5>技术组合</h5><div className="job-tag-list">{candidateFacts.technology_names.map((name) => <span key={name}>{name}</span>)}</div></> : null}
                {/* 招聘侧的两个数字要一起给：各自都常见、合起来一次没出现过，缺口才成立。
                    只写「0」会被读成没查到数据，所以把它连同各自的提及量一并陈述。 */}
                {candidateFacts.jd_cooccurrence !== null && candidateFacts.jd_cooccurrence !== undefined ? <><h5>招聘侧共现</h5><dl className="job-field-grid">{Object.entries(candidateFacts.jd_mentions).map(([code, count]) => <div key={code}><dt>{code}</dt><dd>{count} 份 JD</dd></div>)}<div><dt>同时要求</dt><dd>{candidateFacts.jd_cooccurrence} 份 —— 这正是缺口本身</dd></div></dl></> : null}
                {candidateFacts.milestones.length ? <><h5>依据的产业事件</h5><div className="connected-node-list">{candidateFacts.milestones.map((item) => <div key={`${item.name}-${item.event_date}`} className="candidate-milestone-row"><span>{item.name}<small>{item.event_date ?? '日期未标注'}</small></span></div>)}</div></> : null}
                <p className="graph-legend-note">{classificationGuidance[selectedNode.classification_code ?? ''] ?? '该候选为未入库的提议，处置以审核台判定为准。'}</p>
              </section> : null}
