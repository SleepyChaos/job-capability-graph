...
function EnterpriseDetail({
  ...
  return <div className="enterprise-detail">
    ...
    <section><h4><BriefcaseBusiness size={15} />具体招聘岗位与JD证据</h4>{jobs.length ? <ul className="job-cluster-job-list">{jobs.slice(0, 8).map((job) => <li key={job.id}><button type="button" className="job-landing-link" onClick={() => onJob(job)}><strong>{job.title}</strong><span>{job.standardRoleName || '标准岗位待映射'}<ChevronRight size={13} /></span></button></li>)}</ul> : <p className="job-cluster-muted">该企业暂无可展示的岗位JD证据。</p>}</section>
  </div>
}
...
export function JobEcosystemPage({ fixedView }: { fixedView?: ViewMode }) {
  ...
  return <div className="page-stack job-ecosystem-page">
    ...
    {viewMode === 'industry' ? <div className="job-ecosystem-workspace">
        <Panel title={enterprise ? `${enterprise.name} · 标准岗位需求` : enterpriseValue ? `${enterpriseValue} · 重点企业` : `${dimensionConfig.label} · 企业岗位分布`} subtitle={enterprise ? '企业节点继续下钻到标准岗位；具体JD在右侧' : enterpriseValue ? '图中展示该维度岗位量靠前的企业' : '点击属性值继续下钻'} className="job-ecosystem-graph-panel">
